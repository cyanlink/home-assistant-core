...
class AuthStore:
    ...
    def async_update_user_credentials_data(
        ...
        self._async_schedule_save()
    def _load_groups(self, data: dict) -> dict[str, models.Group]:
        ...
        for group_dict in data.get("groups", []):
            ...
            SYSTEM_GROUPS = {
                GROUP_ID_ADMIN: (GROUP_NAME_ADMIN, system_policies.ADMIN_POLICY),
                GROUP_ID_USER: (GROUP_NAME_USER, system_policies.USER_POLICY),
                GROUP_ID_READ_ONLY: (GROUP_NAME_READ_ONLY, system_policies.READ_ONLY_POLICY)
            }
            if group_id in SYSTEM_GROUPS:
                name, policy = SYSTEM_GROUPS[group_id]
        ...
        self._migrate_users_to_admin_group = not groups and group_without_policy is None
        ...
        if groups and group_without_policy is not None:
            group_without_policy = None
        self._group_without_policy = group_without_policy
        # This is part of migrating from state 1 and 2

        for factory in (_system_admin_group, 
                        _system_user_group, 
    ...
    def _load_users(self, data, groups)-> dict[str, models.User]:
        ...
        for user_dict in data["users"]:
            ...
            for group_id in user_dict.get("group_ids", []):
                # This is part of migrating from state 1
                if group_id == self._group_without_policy:
                    group_id = GROUP_ID_ADMIN
                if group_id in groups:
                    user_groups.append(groups[group_id])

            # This is part of migrating from state 2
            if not user_dict["system_generated"] and self._migrate_users_to_admin_group:
        ...
        return users
    def _load_credentials(self, data, users) -> dict[str, models.Credentials]:
        ...
        return credentials
    def _create_refresh_token(self, rt_dict, user, credentials) -> models.RefreshToken:
        ...
        return token
    def _load_refresh_tokens(self, data, users, credentials):
        for rt_dict in data["refresh_tokens"]:
            ...
            token = self._create_refresh_token(rt_dict, user, credentials)
    ...
    async def async_load(self) -> None:
        ...
        groups = self._load_groups(data)
        users = self._load_users(data, groups)
